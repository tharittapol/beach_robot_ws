...
# ติดตั้ง CUDA 13.0 จาก NVIDIA repo
...
sudo apt update
```
## ติดตั้ง CUDA 13.0 toolkit:
```bash
sudo apt install -y cuda-toolkit-13-0
```
## ตั้ง PATH/LD_LIBRARY_PATH ให้ใช้ CUDA 13.0 เป็นค่าเริ่มต้น
ใส่ใน ~/.bashrc:
```bash
...
nvcc --version
```
## ทำให้ /usr/local/cuda ชี้ไป CUDA 13.0
```bash
...
## ตั้ง env ให้ CMake หา ZED SDK ได้แน่นอน
```bash
...
## อัปเดต zed-ros2-wrapper ให้เป็น v5.2.0
```bash
...
colcon build --symlink-install \
  --executor sequential \
  --cmake-args -DCMAKE_BUILD_TYPE=Release -DZED_DIR=/usr/local/zed
